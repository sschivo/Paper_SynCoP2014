...
which are not directly shown to the user. Indeed, ANIMO was designed with the aim of enabling expert biologists
to formalize their knowledge without the need for additional mathematical training.

\begin{figure}
\begin{center}
  \includegraphics[width=.6\textwidth]{images/animo-ui}
...
\begin{description}
  ...
  \item[Scenario 2] differently from scenario 1, scenario 2 models the interaction rate
...
and scenario choices other than 1 leads to the graph shown in Figure~\ref{fig:animo-ui}.
...
A logarithmic-scale was selected, letting the parameter values
vary over the 5 pre-set values ``very slow'' ($k = 0.001$), ``slow'' ($k = 0.002$), \dots, ``very fast'' ($k = 0.016$)
for all 5 interactions in the network. This resulted in $5 ^ 5 = 3125$ simulations, that were performed
in 34 seconds on an eight-core Intel\circledR\ Core$^{\mbox{\scriptsize\texttrademark}}$ i7 CPU at 2.80GHz equipped with 4 Gb RAM
and running Ubuntu GNU/Linux 13.10 64bit. The result window shown by ANIMO at the end of the computation can
...
\begin{enumerate}
  ...
  \item When a model is obtained that behaves more closely to the experimental data, make it more precisely fitting
    going back to step~\ref{step:precise-fit}.
\end{enumerate}
