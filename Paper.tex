...
This allows us to make a more exhaustive search than the one presented in Figure~\ref{fig:small-example-feedback},
using an automatic approach instead of a manual search.
A logarithmic parameter sweep was performed, letting the parameter values
vary over the 5 pre-sets ``very slow'' ($k = 0.001$), ``slow'' ($k = 0.002$), \dots, ``very fast'' ($k = 0.016$)
...
makes the sheer number of tasks challenging for any processing unit.
...
Indeed, a more complex model involving 10 interactions instead of 5 would require $5^{10} = 9\,765\,625$ simulations,
which could be computed in approximately a day and a half on an eight-core machine. Yet, such an
