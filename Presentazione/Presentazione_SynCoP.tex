...
\only<1-1>{\Huge How do we set the parameters?}\only<2->{\Large How do we set the parameters?}
\uncover<2->{\normal ANIMO requires less parameters than ODEs,\\but we still need to estimate them}
}
